    ...
    async createNote({ request, response, auth }: HttpContext) {
        ...
        try {
            ...
            if (network.userId !== auth.user!.id) {
                return response.unauthorized({ message: 'You are not authorized to create a note in this network' })
            }
            ...
            if (noteData.parentId) {
                ...
                if (Number(parentFolder.networkId) !== noteData.networkId) {
                    console.log('Parent folder networkId:', typeof parentFolder.networkId, ' Note networkId:', typeof noteData.networkId)
    ...
    async getTopLevelNotesByNetwork({ request, response, auth }: HttpContext) {
        ...
        try {
            const notes = await Note.query()
                ...
                .whereNull('parentId') // Fetch only top-level notes
