...
export default class FoldersController {
    ...
    try {
      ...
      if (network.userId !== auth.user!.id) {
        return response.unauthorized({
          message: 'You are not authorized to create a folder in this network',
        })
      }
  ...
  async updateFolder({ params, request, response, auth }: HttpContext) {
    ...
    try {
      ...
      if (network.userId !== auth.user!.id) {
        return response.unauthorized({
          message: 'You are not authorized to update a folder in this network',
        })
      }
